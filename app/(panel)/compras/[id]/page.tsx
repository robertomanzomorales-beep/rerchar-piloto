    ...
    <div className="supply-columns">
      <div>
        ...
        {purchase.status === "aprobada" && <section className="section-card"><div className="section-heading"><div><div className="eyebrow">ORDEN DE COMPRA</div><h2><ReceiptText size={19}/> Proveedor y seguimiento</h2></div></div>
          <form action={orderPurchaseAction} className="form-grid"><input type="hidden" name="request_id" value={id}/><label>Empresa emisora<select name="issuer" defaultValue="e_y_j"><option value="e_y_j">E y J Limitada</option><option value="rerchar">RERCHAR Chile SpA</option></select></label><label>Proveedor<input name="supplier" required minLength={2} maxLength={160}/></label><label>RUT proveedor<input name="supplier_tax_id" maxLength={20}/></label><label>Dirección proveedor<input name="supplier_address" maxLength={250}/></label><label>Referencia de orden<input name="order_reference" required minLength={2} maxLength={80} placeholder="OC-2026-001"/></label><label>Condiciones de pago<input name="payment_terms" defaultValue="Crédito 30 días" required minLength={2} maxLength={120}/></label><label>Fecha esperada<input type="date" name="expected_date"/></label><label>IVA<select name="vat_rate" defaultValue="0.19"><option value="0.19">19%</option><option value="0">0%</option></select></label><div className="form-section-title field-full">Precio por artículo en CLP</div>{lines.map(line=><label key={line.id}>{line.name} ({Number(line.quantity).toLocaleString("es-CL")} {line.unit})<input name={`price_${line.id}`} type="number" min="0" step="0.01" required placeholder="Precio unitario"/></label>)}<div className="field-full form-footer"><SubmitButton className="button button-primary">Emitir orden de compra</SubmitButton></div></form>
        </section>}
