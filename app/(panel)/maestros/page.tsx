...
export default async function Masters({ searchParams }: { searchParams: Promise<{ error?: string; ok?: string }> }) {
  ...
  const [clients, sites, assets, users] = await Promise.all([
    db.query<{ id: string; name: string; tax_id: string | null; contact_name:string|null }>("SELECT id,name,tax_id,contact_name FROM clients WHERE deleted_at IS NULL ORDER BY name"),
