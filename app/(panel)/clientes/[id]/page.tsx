...
export default async function ClientDossier({params}:{params:Promise<{id:string}>}){
  ...
  if(!client)notFound();
  const [services,documents,certificates,invoices,quotes,receipts]=await Promise.all([
    ...
    db.query<{id:string;movement_on:Date|string;kind:string;material_name:string;weighing_ticket:string}>(`SELECT id,movement_on,kind,material_name,weighing_ticket
      FROM material_receipts WHERE client_id=$1 ORDER BY movement_on DESC LIMIT 200`,[id]),
  ]);
  ...
  return <><Link className="back-link" href="/clientes">← Clientes</Link><div className="page-title-row"><div><div className="eyebrow">CARPETA DEL CLIENTE</div><h1>{client.name}</h1><p className="page-intro">{[client.tax_id,client.contact_name,client.email,client.address].filter(Boolean).join(" · ")||"Sin datos adicionales"}</p></div><span className="title-icon"><Building2 size={23}/></span></div>
    ...
    <section className="section-card"><h2>Ingresos, excedentes y hojas de entrada</h2><ul className="record-list">{receipts.map(r=><li key={r.id}><Link href={`/ingresos/${r.id}`}><strong>{r.weighing_ticket} · {r.material_name}</strong></Link><small>{r.kind} · {dateOnly(r.movement_on)}</small></li>)}{!receipts.length&&<li className="muted">Sin movimientos de material cargados.</li>}</ul></section>
