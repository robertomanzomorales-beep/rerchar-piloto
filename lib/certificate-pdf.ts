...
export async function renderCertificatePdf(certificate:PrintableCertificate,verificationUrl:string){
  ...
  page.drawRectangle({x:45,y:425,width:505,height:147,color:pale});
  field("Cliente",certificate.snapshot.client_name,60,546,222);
  field("Centro o faena",certificate.snapshot.site_name,307,546,220);
  field("Tipo de servicio",certificate.snapshot.service_type,60,499,222);
  field("Residuo",certificate.snapshot.waste_type,307,499,220);
  field("Peso neto registrado",`${Number(certificate.snapshot.quantity_kg).toLocaleString("es-CL")} kg`,60,452,222);
  field("Guía de respaldo",certificate.snapshot.guide_number,307,452,220);
  write("TRAZABILIDAD REGISTRADA",45,404,11,bold,green,450);
  page.drawLine({start:{x:45,y:394},end:{x:550,y:394},thickness:1,color:line});
  field("Generador",certificate.snapshot.generator_name,45,374,235);
  field("Transportista",certificate.snapshot.transporter_name,303,374,245);
  field("Receptor",certificate.snapshot.receiver_name,45,325,235);
  field("Destino / tratamiento informado",certificate.snapshot.treatment,303,325,245);
  field("Clasificación revisada",certificate.snapshot.classification,45,276,235);
  field("Categoría informada",certificate.snapshot.category.replaceAll("_"," "),303,276,245);
  page.drawRectangle({x:45,y:81,width:505,height:153,color:rgb(0.97,0.98,0.97),borderColor:line,borderWidth:1});
  ...
  page.drawImage(qr,{x:60,y:94,width:127,height:127});
  write("VERIFICACIÓN DIGITAL",206,197,11,bold,green,290);
  write("Escanee el código para consultar vigencia y versión.",206,173,9,regular,ink,326);
  write(verificationUrl,206,149,8,regular,muted,330);
  write(`Código: ${certificate.code.slice(0,16).toUpperCase()}...`,206,121,8,bold,ink,320);
  page.drawLine({start:{x:45,y:64},end:{x:550,y:64},thickness:1,color:line});
  write("Constancia basada en los registros revisados del servicio. No reemplaza declaraciones oficiales ni documentos de terceros.",45,48,7,regular,muted,507);
  write("RERCHAR Industrial Waste Management System  |  Diseñado y potenciado por vialoop.cl",45,31,7,bold,muted,507);
  return doc.save();
}
