...
export default function RequestForm({ clients, sites, clientId, submissionKey }: { clients: Client[]; sites: Site[]; clientId?: string; submissionKey: string }) {
  ...
  return <form action={createServiceAction} className="form-grid">
    ...
    <label>Tipo de servicio<select name="service_type" defaultValue="retiro" required><option value="retiro">Retiro</option><option value="traslado">Traslado</option><option value="compra">Compra</option><option value="venta">Venta</option><option value="otro">Otro</option></select></label>
